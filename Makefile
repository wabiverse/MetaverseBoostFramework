...
#
# set minimum macOS version supported
#
...
ifneq ($(findstring $(XROS_SDK), $(SDK_NAME)),)
	SDK = $(XROS_SDK)
else
	ifneq ($(findstring $(XRSIMULATOR_SDK), $(SDK_NAME)),)
		SDK = $(XRSIMULATOR_SDK)
	else
...
#
# $1 - sdk (macosx, xros, xrsimulator, iphoneos, or iphonesimulator)
...
$(MAKER_BUILDROOT_DIR)/$(1)/$(2)/$(FRAMEWORKBUNDLE)$(INSTALLED_LIB) :
	...
	./b2 --build-dir="$$$$builddir" --prefix="$$$$installdir" $$(JAM_OPTIONS) toolset=clang-darwin-$(3) target-os=darwin warnings=off link=static $$(JAM_PROPERTIES) install && \
...
$(eval $(call configure_template,$(XROS_SDK),$(ARM_64_ARCH),arm64))
$(eval $(call configure_template,$(XRSIMULATOR_SDK),$(ARM_64_ARCH),arm64))
...
$(SDK_FRAMEWORK_DIR)/Info.plist :
	...
	$(at)/usr/libexec/plistbuddy -c "Set:CFBundleIdentifier foundation.wabi.$(NAME)" $@
