...
node('osx && ios') {
    ...
    try {
        ...
        stage ('Build') {
            ...
            sh 'make ENABLE_BITCODE=YES BITCODE_GENERATION_MODE=bitcode SDK=xros install'
            sh 'make ENABLE_BITCODE=YES BITCODE_GENERATION_MODE=bitcode SDK=xrsimulator install'
...
def getUtils() {
    try {
        ...
        checkout([$class: 'GitSCM',
            ...
            submoduleCfg: [],
            userRemoteConfigs: [[url: 'git@github.com:wabiverse/JenkinsUtils.git', credentialsId: '38bf8b09-9e52-421a-a8ed-5280fcb921af']]])
    } catch(err) {
